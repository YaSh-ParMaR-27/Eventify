...
import razorPayLogo from '../assets/aboutus.svg'
...
export default function EventCheckout() {
  ...
  const displayRazorpay = async () => {
    ...
    const res = await fetch('/razorpay',{
      ...
      body:JSON.stringify({name:location.state.userData.name, email:location.state.userData.email, mobile:location.state.userData.mobile, event_name:location.state.eventData.cardTitle}),
      credentials:"include"
    })
    if(res.status === 409){
          ...
          <AccordionDetails>
            <div className='px-6'>
              <table style={{ width: "100%" }}>
                ...
                <tbody>
                  <tr className='flex justify-between'>
                    <td>{location.state.eventData.cardTitle}</td>
                    <td><CurrencyRupeeIcon fontSize='12' />{location.state.eventData.cardPrice}</td>
                  </tr>
                  <tr className='flex justify-between'>
                    <td colSpan={2}>{location.state.eventData.cardDate} | {location.state.eventData.cardTime} </td>
                  </tr>
                  <hr style={{ border: "1px dashed black", marginTop: "4px", marginBottom: "4px" }} />
                  <tr className='flex justify-end'>
                    <td>+ Booking Charge </td>
                    <td className='ml-[5%]'><CurrencyRupeeIcon fontSize='1' />{location.state.eventData.cardPrice === 0 ? 0 : 1}</td>
                  </tr>
                  <tr className='flex justify-end'>
                    <td>Total</td>
                    <td className='ml-[2%]'><CurrencyRupeeIcon fontSize='1' />{location.state.eventData.cardPrice === 0 ? 0 :  location.state.eventData.cardPrice+1}</td>
                  </tr>
                  <hr style={{ border: "1px dashed black", marginTop: "4px", marginBottom: "4px" }} />
                  <tr className='flex justify-center'>
                    <Button onClick={location.state.eventData.cardPrice === 0 ? registerWithoutPay : displayRazorpay } variant='contained'>Continue</Button>
                  </tr>
                </tbody>
              </table>
            </div>
